...
import { playCentreTick, playCollect, playCollision, playInRange, playPing } from '../audio/cues'
...
export class World {
  ...
  start(parameters: DifficultyParameters): void {
    ...
    for (let i = 0; i < parameters.hazardCount; i++) {
      ...
      const hazard: Hazard = { x: spot.x, y: spot.y, source: null, armed: true }
  ...
  resume(): void {
    ...
    for (const hazard of this.hazards) hazard.source = this.pool.acquire(hazardVoice())
  }
  ...
  ping(): void {
    ...
    for (const entity of this.voicedEntities()) {
      ...
      const distance = Math.hypot(entity.x - this.player.x, entity.y - this.player.y)
  ...
  spawnTarget(): void {
    ...
    this.targetInRange = false
    if (!this.target.source) this.target.source = this.pool.acquire(targetVoice())
  }
  ...
  private updateHazards(): void {
    ...
    for (const hazard of this.hazards) {
      ...
      const distance = Math.hypot(hazard.x - this.player.x, hazard.y - this.player.y)
      if (hazard.armed && distance <= radius) {
        hazard.armed = false
        playCollision()
        this.events.onHazardHit?.()
      } else if (!hazard.armed && distance > radius + GAME.HAZARD_REARM_MARGIN) {
  ...
  private updateWall(): void {
    ...
    for (const option of options) if (option[0] < nearest[0]) nearest = option
    ...
    if (distance <= CUES.WALL.AUDIBLE_DISTANCE) {
      if (!this.wall.source) this.wall.source = this.pool.acquire(wallVoice())
  ...
  private syncListener(): void {
    ...
    setListenerPose({ x, y: 0, z: -y }, { x: Math.sin(heading), y: 0, z: -Math.cos(heading) })
  }
  ...
  private randomSpawnPoint(): { x: number; y: number } {
    ...
    const minFromPlayer = this.parameters?.minSpawnDistance ?? GAME.MIN_SPAWN_DISTANCE
    ...
    for (let attempt = 0; attempt < GAME.SPAWN_ATTEMPTS; attempt++) {
      ...
      if (Math.hypot(x - this.player.x, y - this.player.y) < minFromPlayer) continue
      if (placed.some((e) => Math.hypot(x - e.x, y - e.y) < GAME.MIN_ENTITY_SEPARATION)) continue
      return { x, y }
    }
