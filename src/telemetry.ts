...
export class TelemetryRecorder {
  ...
  sample(dt: number, sample: TelemetrySample): void {
    ...
    if (distance < hunt.closestApproach) {
      ...
      if (distance <= this.difficulty.collectRadius * TELEMETRY.OVERSHOOT_ARMING_RADIUS) {
        hunt.armed = true
      }
  ...
  recordCollect(): void {
    ...
    this.acquisitions.push({
      ...
      angularErrorAtWalkStart:
        hunt.angularErrorAtWalkStart === null ? null : round3(hunt.angularErrorAtWalkStart),
      closestApproach: round3(hunt.closestApproach === Infinity ? 0 : hunt.closestApproach),
      overshootDistance: round3(Math.max(0, hunt.furthestSinceClosest - hunt.closestApproach)),
    })
    this.hunt = newHunt()
  }
...
function append(record: RoundRecord): void {
  try {
    ...
    window.localStorage.setItem(TELEMETRY.STORAGE_KEY, JSON.stringify(trimmed))
  } catch {
    // See loadSessions.
  }
}
