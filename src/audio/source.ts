...
export class BinauralSource {
    ...
    acquire(voice: SourceVoice): void {
        ...
        this.panner.rolloffFactor =
            voice.rolloffFactor ?? AUDIO.PANNER.rolloffFactor;
        ...
        this.oscGain.gain.setTargetAtTime(
            wantsNoise ? 0 : 1,
            now,
            AUDIO.TIMBRE_FADE
        );
    ...
    pulseOnce(): void {
        ...
        this.nextPulseTime = Math.max(
            this.nextPulseTime,
            now + length * AUDIO.PULSE_ONCE_SPACING
        );
    }
    ...
    update(): void {
        ...
        if (voice.maxCutoffHz !== undefined)
            cutoff = Math.min(cutoff, voice.maxCutoffHz);
    ...
    private schedulePulses(
        ...
        let guard = 0;
        while (
            this.nextPulseTime < horizon &&
            guard++ < AUDIO.MAX_PULSES_PER_UPDATE
        ) {
    ...
    private emitPulse(at: number, length: number): void {
        ...
        const attack = Math.min(
            AUDIO.PULSE_ATTACK,
            length * AUDIO.PULSE_ATTACK_MAX_RATIO
        );
