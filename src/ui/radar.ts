...
export class Radar {
  ...
  render(view: RadarView | null): void {
    ...
    if (view.target) {
      ...
      if (view.inRange) this.ring(tx, ty, RADAR.TARGET_RADIUS + RADAR.IN_RANGE_RING_GAP, this.palette.green)
    }
  ...
  private advanceSweep(): void {
    ...
    this.sweep = (this.sweep + ((now - previous) / 1000) * RADAR.SWEEP_SPEED) % 360
  }
  ...
  private drawSweep(inner: number): void {
    ...
    ctx.lineTo(centre + Math.sin(angle) * radius, centre - Math.cos(angle) * radius)
    ctx.stroke()
    ctx.restore()
  }
